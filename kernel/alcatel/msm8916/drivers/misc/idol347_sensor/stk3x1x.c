...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	250
	#define STK_LT_N_CT	70
	#define STK_HT_N_CT	200
#endif	/* #ifdef STK_TUNE0 */

//#define STK_PT_MODE
#ifdef STK_PT_MODE
	#define STK_LT_DEF     1020
	#define STK_HT_DEF     1050
	#define STK_LT_PT      400
	#define STK_HT_PT      450
#else
	#define STK_LT_DEF     65534	//1020
	#define STK_HT_DEF     65535	//1050
	#define STK_LT_PT      600	//400
	#define STK_HT_PT      650	//450
#endif
...
struct input_dev *stk_proximity_dev;
...
*/		
	...
	w_state_reg = stk3x1x_get_state(ps_data);
	printk(KERN_INFO "%s: state_reg = 0x%x\n", __func__, w_state_reg);
...
static int stk3x1x_chk_reg_valid(struct stk3x1x_data *ps_data) 
{
	...
	*/
	...
	if((value[1] != ps_data->alsctrl_reg) && (value[1] != (ps_data->alsctrl_reg - 2)))
	{
...
static int stk_get_offset(struct stk3x1x_data *ps_data, uint16_t ct_value)
{
	if(ct_value <= 200)
	{
		ps_data->stk_max_min_diff = 250;
		ps_data->stk_ht_n_ct = 250;
		ps_data->stk_lt_n_ct = 130;
	}
	else if(ct_value <= 600)
	{
		ps_data->stk_max_min_diff = 260;
		ps_data->stk_ht_n_ct = 260;
		ps_data->stk_lt_n_ct = 140;
	}
	else if(ct_value <= 1000)
	{
		ps_data->stk_max_min_diff = 450;
		ps_data->stk_ht_n_ct = 270;
		ps_data->stk_lt_n_ct = 150;
	}
	else
	{
		ps_data->stk_max_min_diff = 500;
		ps_data->stk_ht_n_ct = 280;
		ps_data->stk_lt_n_ct = 160;
	}
...
static int stk_ps_tune_zero_final(struct stk3x1x_data *ps_data)
{
...
#ifdef CALI_PS_EVERY_TIME
	#ifdef STK_PT_MODE
	if((ps_data->ps_stat_data[1] > STK_LT_DEF)||(data < 2))
	{
		ps_data->ps_high_thd_boot = STK_HT_DEF;
		ps_data->ps_low_thd_boot = STK_LT_DEF;
		ps_data->boot_cali = 0;
	...
	{
		ps_data->ps_high_thd_boot = ps_data->ps_stat_data[1] + STK_HT_PT;
		ps_data->ps_low_thd_boot = ps_data->ps_stat_data[1] + STK_LT_PT;
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_data *ps_data)
{
	...
	if((ps_data->psa - ps_data->psi) > ps_data->stk_max_min_diff)
	{
		#ifdef STK_PT_MODE
		...
		if(ps_data->boot_ct > ps_data->psi)
			ps_data->boot_ct = ps_data->psi;
		else if(ps_data->boot_ct <= ps_data->psi)
			ps_data->psi = ps_data->boot_ct;
	...
	//	ps_data->ps_high_thd_boot = ps_data->psi + STK_HT_PT;
	//	ps_data->ps_low_thd_boot = ps_data->psi + STK_LT_PT;
		...
		if(ps_data->ps_thd_h > ps_data->ps_high_thd_tmp)
		{
			if((ps_data->boot_cali >= 1)&&(ps_data->boot_ct != 0xFFFF))
			{
				ps_data->ps_high_thd_boot = ps_data->boot_ct + STK_HT_PT;
				ps_data->ps_low_thd_boot = ps_data->boot_ct + STK_LT_PT;
		...
		*/
		...
		{
			ps_data->boot_ct = ps_data->psi;
			ps_data->ps_high_thd_tmp = ps_data->boot_ct + STK_HT_PT;
			ps_data->ps_low_thd_tmp = ps_data->boot_ct + STK_LT_PT;
...
//	reading_lux = stk_alscode2lux(ps_data, reading);
	...
	{
		reading_lux = (reading_lux * 100) /48;

	}
	else if((ps_data->ir_code /reading_lux)> SUNLIGHT_IR)
	{
	...
	else if(abs(ps_data->als_lux_last - reading_lux) >= STK_ALS_CHANGE_THD)
	{
	...
	else if(reading < 3 && ps_data->als_lux_init_report == 0)
	{
...
struct input_dev* get_stk_input_device(void)
{
	return stk_proximity_dev;
}
                        ...
                        const struct i2c_device_id *id)
{
	...
	stk_proximity_dev = ps_data->ps_input_dev;
